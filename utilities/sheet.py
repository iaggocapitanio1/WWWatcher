...
@functools.cache
def read_data_module(input_data: Optional[Union[str, Path, DataFrame]] = None) -> list:
    """
    Reads data from the 'DATA - ASM' sheet of an Excel file or from an input DataFrame and returns it as a list.

    Parameters:
    - input_data (Optional[Union[str, Path, pd.DataFrame]]): The file path to the Excel file, which can be a string
        or a pathlib.Path object, or a DataFrame. If None, an empty list is returned.

    Returns:
    - list: A list containing the data from the first column of the 'DATA - ASM' sheet.
            Returns an empty list if the file is not found, the sheet doesn't exist, or any other error occurs.
    """
    try:
        if isinstance(input_data, DataFrame):
            ...
            data_frame = read_excel(input_data, sheet_name='DATA - ASM', usecols=[0])

        data_list = data_frame.iloc[:, 0].tolist()
        return data_list

    except FileNotFoundError:
        logger.error(f"The file at '{input_data}' was not found.")
    except Exception as e:
        # This will catch any other exception including if the sheet doesn't exist
